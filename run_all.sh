...
cg_tol=1e-2 # Relative tolerance of conjugate gradient solver
CFL=3.0
...
./apps/FreeFUS/FreeFUS --mesh BP1 --material-case 1 --source-frequency ${source_frequency} --cg-tol ${cg_tol} --CFL ${CFL} --sample-harmonic ${sample_harmonic} --sample-periods ${sample_periods} --buffer-periods ${buffer_periods} --insitu-output-steps 10000 --output-steps 10000 --output-path output-BP1 | tee BP1.log
