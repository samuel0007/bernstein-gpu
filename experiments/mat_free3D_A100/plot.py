...
P = list(range(3, 10))
sf, sf_otf, base = zip(*(parse_log_file(f'log_float64_{i}.txt') for i in P))
...
plt.ylabel('GDoFs/s')
plt.title('Mat-free performance on A100 \n 3D mass on tetrahedral mesh, 64bit')
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.savefig('A100_mat_freeb3D_64B.png')
