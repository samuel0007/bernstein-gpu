...
unpack_add(std::int32_t N, const int32_t *__restrict__ indices,
           const T *__restrict__ in, T *__restrict__ out) {
  int gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < N) {
...
enum class Device { CUDA, HIP, CPP };
...
/// Distributed vector
template <typename T, Device D> class Vector {
public:
  /// The value type
  using value_type = T;
  ...
  template <typename OtherVector> void copy(OtherVector &other) {
...
void copy_d_to_d(auto &in, auto &out) {
...
/// @note Collective MPI operation
template <typename Vector> auto squared_norm(const Vector &a) {
  using T = typename Vector::value_type;
...
/// @param r Result
/// @param alpha
template <typename Vector, typename S> void scale(Vector &r, S alpha) {
  using T = typename Vector::value_type;
...
/// @param a
/// @param b
template <typename Vector> void copy(Vector &a, const Vector &b) {
  using T = typename Vector::value_type;
  const std::int32_t local_size = a.bs() * a.map()->size_local();
...
void pointwise_mult(Vector &w, const Vector &x, const Vector &y) {
  ...
  thrust::transform(
      ...
      x.array().begin() + x.map()->size_local(), y.array().begin(),
      w.mutable_array().begin(),
