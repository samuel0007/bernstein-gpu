...
namespace dolfinx::acc
{
  ...
  public:
    ...
    MatFreeMassExteriorBaseline(std::shared_ptr<mesh::Mesh<T>> mesh,
                                std::shared_ptr<fem::FunctionSpace<T>> V,
                                std::vector<std::int32_t> cell_facet_data,
                                T alpha)
        : mesh(mesh), V(V), cell_facet_data(cell_facet_data)
    {
      auto dofmap = V->dofmap();
      auto map = dofmap->index_map;
      int map_bs = dofmap->index_map_bs();
    ...
    void init(std::span<const T> alpha)
    {
      ...
      const std::size_t tdim = mesh->topology()->dim();
      this->number_of_local_facets = this->cell_facet_data.size() / 2;
      ...
      auto elem_1d = std::make_shared<basix::FiniteElement<T>>(basix::create_element<T>(
          element_p->basix_element().family(), basix::cell::type::interval, P,
          element_p->basix_element().lagrange_variant(),
          basix::element::dpc_variant::unset, (P == 0)));
      auto [phi_table, shape] =
          elem_1d->tabulate(0, qpts, {qpts.size(), 1});
