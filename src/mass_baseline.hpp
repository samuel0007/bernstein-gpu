...
namespace dolfinx::acc
{
  ...
  public:
    ...
    void init(std::span<const std::int32_t> cell_facet_data, std::span<const T> alpha)
    {
      // auto [lcells, bcells] = compute_boundary_cells(V);
      // spdlog::debug("#lcells = {}, #bcells = {}", lcells.size(),
      // bcells.size());
      ...
      // TODO: one could be smart, and pack the necessary exterior data instead of sending everything.
      // Transfer V dofmap to the GPU
      // Or just use the same pointer as for the normal mass matrix?
      // Like passing the dofmap as argument
      auto dofmap = V->dofmap();
      ...
      alpha_d_span = copy_to_device(
          alpha.begin(), alpha.end(),
          alpha_d, "alpha");

      cell_facet_d_span = copy_to_device(
          cell_facet_data.begin(), cell_facet_data.end(),
      ...
      for (int i = 0; i < n_faces; ++i)
      {
        ...
        auto [phi_table, shape] =
            element_p->tabulate(mapped_points, {mapped_points.size() / tdim, tdim}, 0);
        assert(shape[0] == 1 && shape[3] == 1);
        ...
        for (int q = 0; q < Q; ++q)
        {
          for (int k = 0; k < K; ++k)
          {
            ...
            // std::cout << std::format("facets_phi[{}][{}][{}]={}", i, q, k, phi_table[q * K + k]) << std::endl;
          }
        }
      }
      std::cout << std::format("Table size = {}, Fxqxk: {}x{}x{}", facets_phi_table.size(),
                               n_faces, Q, K)
                << std::endl;
      facets_phi_d_span = copy_to_device(
          facets_phi_table.begin(), facets_phi_table.end(),
          facets_phi_d, "facets phi");

      // Precompute geometry data
      std::vector<T> detJ_geom = compute_geometry_facets(mesh, cell_facet_data, qpts, qwts);
      assert(detJ_geom.size() == this->number_of_local_cells * n_faces * nq);

      detJ_geom_d_span = copy_to_device(
          detJ_geom.begin(), detJ_geom.end(),
      ...
      faces_dofs_d_span = copy_to_device(
          faces_dofs.begin(), faces_dofs.end(),
          faces_dofs_d, "local face index to local closure dofs");
    }
    ...
    void operator()(const Vector &in, Vector &out)
    {
      ...
      facets_mass_operator_baseline<T, nd, nq><<<grid_size, block_size>>>(
          in_dofs, out_dofs,
          this->cell_facet_d_span.data(),
          this->detJ_geom_d_span.data(),
          this->alpha_d_span.data(),
          this->dofmap_d_span.data(),
          this->facets_phi_d_span.data(),
          this->faces_dofs_d_span.data(),
          this->n_faces);
      check_device_last_error();
    }
  ...
  public:
    ...
    void init(std::span<const std::int32_t> cell_facet_data, std::span<const T> alpha)
    {
      ...
      std::cout << std::format("Table size = {}, Fxnqxnd: {}x{}x{}", facets_phi_table.size(),
                               n_faces, nq, nd)
                << std::endl;
      facets_phi_d_span = copy_to_device(
          facets_phi_table.begin(), facets_phi_table.end(),
  ...
  private:
    ...
    static constexpr int nq = 6 * (Q == 3) + 12 * (Q == 4) + 16 * (Q == 5) +
                              25 * (Q == 6) + 33 * (Q == 7) + 42 * (Q == 8) +
                              55 * (Q == 9) + 67 * (Q == 10) + 79 * (Q == 11) +
                              96 * (Q == 12) + 112 * (Q == 13); // 2D rules (on facet)
