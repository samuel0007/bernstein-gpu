...
namespace dolfinx::acc
{
  ...
  public:
    ...
    MatFreeNewmark3D(std::shared_ptr<mesh::Mesh<U>> mesh,
                     std::shared_ptr<fem::FunctionSpace<U>> V, U alpha1, U alpha2, U alpha3)
        : mesh(mesh), V(V)
    {
      auto dofmap = V->dofmap();
      auto map = dofmap->index_map;
      int map_bs = dofmap->index_map_bs();
      la::Vector<T> alpha1_vec(map, map_bs);
      la::Vector<T> alpha2_vec(map, map_bs);
      la::Vector<T> alpha3_vec(map, map_bs);
      alpha1_vec.set(alpha1);
      alpha2_vec.set(alpha2);
      alpha3_vec.set(alpha3);
      init(alpha1_vec.array(), alpha2_vec.array(), alpha3_vec.array());
    }

    MatFreeNewmark3D(std::shared_ptr<mesh::Mesh<U>> mesh,
                     std::shared_ptr<fem::FunctionSpace<U>> V,
                     std::span<const U> alpha1, std::span<const U> alpha2, std::span<const U> alpha3)
        : mesh(mesh), V(V)
    {
      init(alpha1, alpha2, alpha3);
    }

    void init(std::span<const U> alpha1, std::span<const U> alpha2, std::span<const U> alpha3)
    {
      ...
      alpha1_d_span = copy_to_device(alpha1.begin(), alpha1.end(), alpha1_d, "alpha1");
      alpha2_d_span = copy_to_device(alpha2.begin(), alpha2.end(), alpha2_d, "alpha2");
      alpha3_d_span = copy_to_device(alpha3.begin(), alpha3.end(), alpha3_d, "alpha3");
    ...
    void operator()(Vector &in, Vector &out, U dt, U global_coefficient = 1.,
                    GpuStream stream = 0, int bs = nq, int cells_per_block = 1)
    {
      ...
      kernels::fused::fused_newmark<T, nd, nq>
          <<<grid_size, block_size, shmem, stream>>>(
              out_dofs, in_dofs, this->alpha1_d_span.data(),
              this->alpha2_d_span.data(), this->alpha3_d_span.data(),
              this->detJ_geom_d_span.data(), this->geom_d_span.data(),
              this->dofmap_d_span.data(), this->dphi_d_span.data(), dt,
    ...
    template <typename Vector>
    void get_diag_inverse(Vector &diag_inv, U dt, U global_coefficient = 1.)
    {
      diag_inv.set(0.);
      this->get_diag(diag_inv, dt, global_coefficient);
    ...
    template <typename Vector>
    void get_diag(Vector &diag, U dt, U global_coefficient = 1.)
    {
      ...
      mass_diagonal<T, nd, nq><<<grid_size, block_size>>>(
          out_dofs, this->alpha1_d_span.data(), this->detJ_geom_d_span.data(),
          this->dofmap_d_span.data(), this->dphi_d_span.data(), // phi is start of dphi
          global_coefficient);
      // C
      kernels::stiffness::stiffness_operator3D_diagonal<T, nd, nq>
          <<<grid_size, block_size>>>(
              out_dofs, this->alpha2_d_span.data(), this->geom_d_span.data(),
              this->dofmap_d_span.data(), this->dphi_d_span.data() + this->dphi_d_span.size() / 4,
              global_coefficient * gamma * dt);
      // K
      kernels::stiffness::stiffness_operator3D_diagonal<T, nd, nq>
          <<<grid_size, block_size>>>(
              out_dofs, this->alpha3_d_span.data(), this->geom_d_span.data(),
              this->dofmap_d_span.data(), this->dphi_d_span.data() + this->dphi_d_span.size() / 4,
              global_coefficient * beta * dt * dt);
      check_device_last_error();
    }
  ...
  private:
    ...
    thrust::device_vector<T> alpha1_d;
    std::span<const T> alpha1_d_span;

    thrust::device_vector<T> alpha2_d;
    std::span<const T> alpha2_d_span;

    thrust::device_vector<T> alpha3_d;
    std::span<const T> alpha3_d_span;
