...
template <typename T, int P, int Q, typename U = T> class MatFreeStiffness {
public:
  using value_type = T;
  using quad_rule = std::pair<std::vector<T>, std::vector<T>>;

  MatFreeStiffness(std::shared_ptr<mesh::Mesh<U>> mesh,
  ...
  void init(std::span<const U> alpha) {
    ...
    geom_d_span = compute_stiffness_geometry_triangle_GPU<T, U, nq>(
  ...
  template <typename Vector> void operator()(Vector &in, Vector &out, U global_coefficient = 1.) {
    ...
    dim3 block_size(nq);
    kernels::stiffness::stiffness_operator<T, nd, nq>
...
template <typename T, int P, int Q, typename U = T> class MatFreeStiffness3D {
public:
  using value_type = T;
  using quad_rule = std::pair<std::vector<T>, std::vector<T>>;

  MatFreeStiffness3D(std::shared_ptr<mesh::Mesh<U>> mesh,
                     std::shared_ptr<fem::FunctionSpace<U>> V, U alpha)
      : mesh(mesh), V(V) {
    auto dofmap = V->dofmap();
    auto map = dofmap->index_map;
