...
namespace freefus
{
  ...
  private:
    ...
    void update_source(U t)
    {
      ...
      const U dsource = dwindow * p0 * w0 / source_sound_speed * cos(w0 * t) -
                        window * p0 * w0 * w0 / source_sound_speed * sin(w0 * t);
      g->set(source);
      gd->set(dsource);
    }
  };
  ...
  template <typename U, int P>
  U compute_dt(U h, U sound_speed, U period, U CFL)
  {
  ...
  template <class U>
  void log_progress(int step, int total_steps, U dt, U current_time, U final_time,
                    const TimePoint &start)
  {
    U fraction = static_cast<U>(step) / static_cast<U>(total_steps);
    auto now = Clock::now();
    U elapsed = std::chrono::duration<U>(now - start).count();
    U eta = (fraction > U(0)) ? elapsed * (U(1) - fraction) / fraction : U(0);

    spdlog::info("Step {:4d}/{:4d} — dt = {:10.6e}s — t = {:10.6e}/{:10.6e} ({:5.1f}%) "
                 "— elapsed {:6.2f}s — ETA {:6.2f}s",
                 step, total_steps, double(dt),
