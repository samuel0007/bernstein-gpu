...
int main(int argc, char *argv[]) {
  ...
  PetscInitialize(&argc, &argv, nullptr, nullptr);
  {
    ...
    auto coord_element =
        fem::CoordinateElement<T>(mesh::CellType::hexahedron, 1);
    ...
    if (mpi_rank == 0) {
      std::cout << "Problem type: Planar Transducer 3D" << "\n";
    ...
    auto model = LinearSpectral<T, degreeOfBasis, DeviceVector>(
        mesh, V, mt_facet, c0, rho0, sourceFrequency, sourceAmplitude,
        speedOfSound);
    ...
    dolfinx::io::VTXWriter<T> f_out(mesh->comm(), output_filepath, {u_out},
                                    "bp5");
