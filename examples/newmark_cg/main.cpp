...
public:
  FusedJacobianStream(auto mesh, auto V, auto alpha, auto facet_domain, int bs,
                      int cells_per_block)
      : bs(bs), cells_per_block(cells_per_block) {
  ...
  template <typename Vector> void operator()(Vector &in, Vector &out) {
    ...
    (*Newmark_action_ptr)(in, out, 1, stream1, bs, cells_per_block);
...
public:
  FusedJacobian(auto mesh, auto V, auto alpha, auto facet_domain, int bs,
                int cells_per_block)
      : bs(bs), cells_per_block(cells_per_block) {
    Newmark_action_ptr = std::make_unique<FusedNewmarkAction>(mesh, V, alpha);
    Mgamma_action_ptr =
  ...
  template <typename Vector> void operator()(Vector &in, Vector &out) {
    ...
    (*Newmark_action_ptr)(in, out, 1., 0, bs, cells_per_block);
...
po::variables_map get_cli_config(int argc, char *argv[]) {
  ...
  desc.add_options()("help,h", "print usage message")
      ...
      ("bs", po::value<int>()->default_value(64), "J")
      ("cells-per-block", po::value<int>()->default_value(8), "J");
  // clang-format on
...
void solver(MPI_Comm comm, po::variables_map vm) {
  ...
  FusedJacobian<T, polynomial_degree, quadrature_points> fj_action(
      mesh, V, 1.0, facet_domain, bs, cells_per_block);
