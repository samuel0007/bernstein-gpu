...
void solver(MPI_Comm comm, po::variables_map vm)
{
  ...
  auto mesh = std::make_shared<mesh::Mesh<T>>(mesh::create_box<T>(
      comm, {{{0.0, 0.0, 0.0}, {1.0, 1.0, 1.0}}}, {5, 5, 5}, mesh::CellType::tetrahedron,
  ...
  //     fem::create_form<T>(*form_stiff_cg_M, {V}, {{"ui", ui}}, {{}}, {}, {}));
  // auto L = std::make_shared<fem::Form<T>>(
  //     fem::create_form<T>(*form_stiff_cg_L, {V}, {{"f", f}}, {}, {}, {}));
  ...
  std::cout << "polynomial_degree=" << polynomial_degree << "\n";
  ...
  //                                                                      1.0);
  acc::MatFreeStiffness3D<T, polynomial_degree, quadrature_points> gpu_action(mesh, V,
                                                                               1.0);
