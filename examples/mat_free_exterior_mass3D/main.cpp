...
#include "mat_free_exterior_mass3D.h"
#include <algorithm>
...
void solver(MPI_Comm comm, po::variables_map vm)
{
...

    {
        ...
        constexpr int K = (N + 2) * (N + 1) * N / 6; // Number of dofs on tetrahedron
    ...
    acc::MatFreeMassExteriorBaseline3D<T, polynomial_degree, quadrature_points>
        gpu_action_baseline(mesh, V, facet_domains[0].second, alpha->x()->array());
